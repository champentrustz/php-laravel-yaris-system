...
<main class="main-container">
    ...
    <div class="main-content">
        ...
        <div class="row">
                                    ...
                                    @foreach($orders as $key => $order)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                    ...
                                                    <form action="{{ url('admin/confirm-order/view-order-session') }}" method="post">
                                                        {{ csrf_field() }}
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body form-type-round">
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <a href="{{url('/admin/confirm-order/confirm-payment/'.$order->id)}}" class="btn btn-bold btn-pure btn-primary">Confirm Payment</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <h4 class="card-title"><span class="fa fa-clock-o"></span> รายการออร์เดอร์ที่รอการจัดส่ง</h4>
                    ...
                    <div class="card-body">
                            ...
                            <table class="table table-striped table-bordered table-responsive-lg" cellspacing="0" data-page-length='50' data-provide="datatables">
                                ...
                                @foreach($ordersWaitDelivery as $key => $orderWaitDelivery)



                                    <tr>
                                        <input type="hidden" name="id_orders[]" value="{{$orderWaitDelivery->id}}">
                                        <td class="text-center">{{$key+1}}</td>
                                        <td>{{$orderWaitDelivery->product->name}}</td>
                                        <td>{{$orderWaitDelivery->number}} ชิ้น</td>
                                        <td>{{($orderWaitDelivery->product->member_price * $orderWaitDelivery->number) + $orderWaitDelivery->product->delivery_price }} บาท</td>
                                        <td>{{$orderWaitDelivery->user->first_name}} {{$orderWaitDelivery->user->last_name}}</td>
                                        <td class="text-warning">ตรวจสอบแล้ว รอการจัดส่ง</td>
                                        <td>{{substr($orderWaitDelivery->created_at,0,10)}}</td>
                                        <td class="text-center">

                                            <form action="{{ url('admin/confirm-order/view-order-session') }}" method="post">
                                                {{ csrf_field() }}
                                                <input type="hidden" name="order_id" value="{{$orderWaitDelivery->id}}">
                                                <button type="submit" class="btn btn-xs btn-info">รายละเอียด</button>
                                            </form>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="{{ url('admin/confirm-order/delivery/'.$orderWaitDelivery->id) }}" class="btn btn-xs btn-success">จัดส่ง</a>
                                        </td>
